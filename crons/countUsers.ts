...
export async function run() {
  ...
  const lastCount = (await kv.get(usersCountKey(yesterday))).value as number || 0;
  await kv.set(usersCountKey(today), users.length);

  const diff = users.length - lastCount;

  const message = diff > 0 ? `📈 今日は${diff}人増えました！` :
                diff < 0 ? `📉 今日は${Math.abs(diff)}人減りました…` :
                `😃 メンバー数は変わりませんでした！`;

  const text = `📊 *${today} メンバー数定点観測*\n デジタル民主主義2030メンバー数: ${users.length}人（前日比: ${diff}人）\n${message}`;
